<template>
  <div>
    <el-breadcrumb separator="|" style="margin-bottom: 15px; margin-left: 5px">
      ...
      <el-breadcrumb-item class="testtypevo">{{
        this.tpName
      }}</el-breadcrumb-item>
      <el-breadcrumb-item class="testtypevo"
        >单选题数量：{{ testTypeVO.radioCount }}</el-breadcrumb-item
      >
      <el-breadcrumb-item class="testtypevo"
        >多选题数量：{{ testTypeVO.multiCount }}</el-breadcrumb-item
      >
      <el-breadcrumb-item class="testtypevo"
        >判断题数量：{{ testTypeVO.judgeCount }}</el-breadcrumb-item
      >
      <el-breadcrumb-item class="testtypevo"
        >总分：{{ testTypeVO.scoreTotal }}</el-breadcrumb-item
      >
    ...
    >
      <!--<el-form-item>
                <el-select v-model="searchForm.subId" placeholder="选择科目" clearable>
                    <el-option v-for="sub in subData"  :label="sub.subName" :value="sub.subId"></el-option>
                </el-select>
            </el-form-item>-->
      <el-form-item>
        <el-select
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-tag v-if="scope.row.testState === 0" type="info" effect="dark"
            >未被添加</el-tag
          >
          <el-tag v-if="scope.row.testState === 1" type="success" effect="dark"
            >已添加</el-tag
          >
        </template>
      </el-table-column>
      <el-table-column prop="icon" label="操作" width="220px">
        <template slot-scope="scope">
          <el-button
            type="text"
            size="small"
            ...
            disabled
          >
            <i class="el-icon-check"></i>添加到试卷
          </el-button>
          <el-button
            type="text"
            @click="tplistadd(scope.row.testId)"
            size="small"
...
export default {
  ...
  methods: {
    ...
    getTestBank() {
      console.log("搜索===>");
      this.$axios.post("/Tplist/getTestbank", this.searchForm).then((res) => {
        console.log("搜索结果==>>");
        console.log(res);
    ...
    //添加试题到试卷
    tplistadd(testId) {
      this.subForm.testId = testId;
      this.subForm.tpId = this.searchForm.tpId;
      console.log("=================>");
      console.log(this.subForm);
      this.$axios.post("/Tplist/save", this.subForm).then((res) => {
        this.$message({
          showClose: true,
          message: "你好，添加已完成",
          type: "success",
