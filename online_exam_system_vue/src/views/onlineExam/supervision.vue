<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="stuName" label="学生姓名" width="176px">
      </el-table-column>
      <el-table-column
        prop="subName"
        label="所属科目"
      ...
      <el-table-column prop="examTotal" label="试卷总分" width="176px">
        <template slot-scope="scope"> {{ scope.row.examTotal }}分 </template>
      </el-table-column>

      <el-table-column prop="examScore" label="试卷成绩" width="176px">
        <template slot-scope="scope"> {{ scope.row.examScore }}分</template>
      </el-table-column>
      <el-table-column prop="examScore" label="离开次数" width="176px">
        <template slot-scope="scope"> {{ scope.row.leaveTimes }}次</template>
      </el-table-column>
      <el-table-column prop="examScore" label="作答时间" width="176px">
        <template slot-scope="scope"> {{ scope.row.startTime }}</template>
      </el-table-column>
      <el-table-column prop="examScore" label="交卷时间" width="176px">
...
export default {
  ...
  methods: {
    ...
    getExamScore() {
      ...
      this.$axios.post("/Score/getScore", this.searchForm).then((res) => {
        console.log("搜索结果==>>");
        console.log(res);
