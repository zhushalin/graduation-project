...
public class TestbankController extends BaseController {
    ...
    public Msg getTestbank(@RequestBody TestBankDTO testBankDTO, HttpSession session){
        System.out.println(testBankDTO);
        LoginVO loginUser = getLoginUser();
        if (RoleTypeEnum.TEACHER.getName().equals(loginUser.getRole())) {
            // 教师只能看到自己的
//            Teacher teacher = tearchService.getById(loginUser.getId());
//            String colName = teacher.getColName();
//
//            QueryWrapper<College> wrapperCollege = new QueryWrapper<>();
//            wrapperCollege.eq("col_name", colName);
//            College college = collegeService.getOne(wrapperCollege);
//            Integer colId = college.getColId();
//
//            QueryWrapper<Major> wrapperMajor = new QueryWrapper<>();
//            wrapperMajor.eq("col_id", colId);
//            Major major = majorService.getOne(wrapperMajor);
//
//            QueryWrapper<Subject> wrapperSubject = new QueryWrapper<>();
//            wrapperSubject.eq("major_id", major.getMajorId());
//            List<Subject> subjectList = subjectService.list(wrapperSubject);
        }
